...
import { Scale, Plus, Pencil, Trash2, TrendingDown, TrendingUp, Minus, Users } from "lucide-react";
import { LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer } from "recharts";
import { useToast } from "@/hooks/use-toast";
import { Link } from "wouter";

type WeightEntry = { id: number; memberId: number; weightKg: number; recordedAt: string; note?: string | null; createdAt: string; };
type FamilyMember = { id: number; name: string; gender: string; };

const schema = z.object({
  weightKg: z.coerce.number().min(0.5).max(600),
...
export default function Weight() {
  ...
  ) as { data: WeightEntry[]; isLoading: boolean };
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: { weightKg: 70, recordedAt: toDatetimeLocal(), note: "" },
  });

  function openAdd() {
    setEditing(null);
    form.reset({ weightKg: entries[0]?.weightKg ?? 70, recordedAt: toDatetimeLocal(), note: "" });
    setDialogOpen(true);
  }
  function openEdit(e: WeightEntry) {
    setEditing(e);
    form.reset({ weightKg: e.weightKg, recordedAt: toDatetimeLocal(e.recordedAt), note: e.note ?? "" });
    setDialogOpen(true);
  }

  function onSubmit(data: FormValues) {
    const payload = { memberId: selectedMemberId!, weightKg: data.weightKg, recordedAt: new Date(data.recordedAt).toISOString(), note: data.note || undefined };
    if (editing) {
      updateEntry.mutate({ id: editing.id, data: { weightKg: data.weightKg, recordedAt: new Date(data.recordedAt).toISOString(), note: data.note ?? null } }, {
        onSuccess: () => { qc.invalidateQueries({ queryKey: getListWeightEntriesQueryKey({ memberId: selectedMemberId! }) }); setDialogOpen(false); toast({ title: "Entry updated" }); },
        onError: (e) => toast({ title: "Error", description: (e as any)?.data?.error || "Failed", variant: "destructive" }),
      });
    } else {
      createEntry.mutate({ data: payload }, {
        onSuccess: () => { qc.invalidateQueries({ queryKey: getListWeightEntriesQueryKey({ memberId: selectedMemberId! }) }); setDialogOpen(false); toast({ title: "Weight entry added!" }); },
        onError: (e) => toast({ title: "Error", description: (e as any)?.data?.error || "Failed", variant: "destructive" }),
      });
    }
  }
  ...
  const chartData = sorted.map(e => ({ date: format(new Date(e.recordedAt), "MMM d"), weight: e.weightKg }));
  const latest = entries[0]?.weightKg;
  const prev = entries[1]?.weightKg;
  const trend = latest !== undefined && prev !== undefined ? latest - prev : null;
  ...
  if (members.length === 0) {
    return (
      <div className="max-w-2xl mx-auto text-center py-16">
        ...
        <p className="text-muted-foreground mb-6">Add family members first to start tracking weight.</p>
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-serif font-semibold text-foreground">Weight Log</h1>
          <p className="text-muted-foreground text-sm mt-1">Track weight over time</p>
        </div>
      ...
      {selectedMemberId && entries.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
          <Card><CardContent className="p-4">
            ...
            <div className="text-2xl font-bold text-foreground">{latest?.toFixed(1)} <span className="text-sm font-normal text-muted-foreground">kg</span></div>
          </CardContent></Card>
          <Card><CardContent className="p-4">
            <div className="text-xs text-muted-foreground uppercase tracking-wide mb-1">Change</div>
            {trend !== null ? (
              <div className={`text-2xl font-bold flex items-center gap-1 ${trend < 0 ? "text-emerald-600" : trend > 0 ? "text-destructive" : "text-muted-foreground"}`}>
            ...
            ) : <div className="text-muted-foreground text-sm mt-2">No change yet</div>}
          </CardContent></Card>
          <Card><CardContent className="p-4">
      ...
      {chartData.length > 1 && (
        <Card>
          <CardHeader><CardTitle className="text-base font-medium">Weight Trend</CardTitle></CardHeader>
          <CardContent>
              ...
              <div className="divide-y divide-border">
                {entries.map(e => (
                  <div key={e.id} className="flex items-center justify-between py-3">
                    <div>
                      <div className="font-semibold">{e.weightKg.toFixed(1)} kg</div>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader><DialogTitle className="font-serif">{editing ? "Edit Entry" : "Add Weight Entry"}</DialogTitle></DialogHeader>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              <FormField control={form.control} name="weightKg" render={({ field }) => (
                <FormItem><FormLabel>Weight (kg)</FormLabel><FormControl><Input type="number" step="0.1" placeholder="70.0" {...field} /></FormControl><FormMessage /></FormItem>
              )} />
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={(o) => !o && setDeleteTarget(null)}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={() => { if (deleteTarget) deleteEntry.mutate({ id: deleteTarget.id }, { onSuccess: () => { qc.invalidateQueries({ queryKey: getListWeightEntriesQueryKey({ memberId: selectedMemberId! }) }); setDeleteTarget(null); } }); }} className="bg-destructive hover:bg-destructive/90">Delete</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </div>
